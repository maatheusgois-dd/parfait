...
final class RecordingSession: ObservableObject {
    ...
    func start(micURL: URL, systemURL: URL) throws {
        ...
        if shouldTrackZoomSpeakers() {
            NutolaConsoleLog.recording("Zoom speaker tracker \(AccessibilityPermission.isTrusted ? "starting" : "waiting for Accessibility")")
            ensureZoomTracker()
    ...
    var startupNotice: String? {
        ...
        if shouldTrackZoomSpeakers(), !AccessibilityPermission.isTrusted {
            parts.append("Enable Accessibility for Nutola in Privacy & Security to label Zoom speakers by name.")
        }
        return parts.isEmpty ? nil : parts.joined(separator: " ")
    }

    func stop() {
        NutolaConsoleLog.recording("stop meeting=\(meetingID.uuidString.prefix(8)) elapsed=\(Int(elapsed))s liveSegments=\(liveSegments.count)")
