...
struct GranolaFloatingPanel: View {
            ...
            if canAskAboutMeeting {
                VStack(alignment: .leading, spacing: 4) {
                    ForEach(MeetingAISuggestions.forMeeting(meeting), id: \.self) { suggestion in
                        Button { launchAsk(suggestion) } label: {
                            HStack(spacing: 10) {
                ...
                Text("Record the meeting or add notes first — then you can ask questions about what happened.")
                    .font(.nutola(12))
                    .foregroundStyle(Theme.secondary(scheme))
    ...
    private var showCollapsedResume: Bool {
        ...
        let summaryEmpty = app.store.summary(for: meeting.id).isEmpty
    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            .onChange(of: session.liveSegments.count) {
                ...
                proxy.scrollTo("granola-live-bottom", anchor: .bottom)
            }
