...
enum CrashDiagnosticLog {
    ...
    /// before `ParfaitApp.main()`. No-op when the user hasn't opted in.
    static func install() {
        ...
        let parfaitDir = base.appendingPathComponent("Parfait", isDirectory: true)
        try? FileManager.default.createDirectory(at: parfaitDir, withIntermediateDirectories: true)
        let crashesDir = parfaitDir.appendingPathComponent("Crashes", isDirectory: true)
        try? FileManager.default.createDirectory(at: crashesDir, withIntermediateDirectories: true)
        diagnosticsPathC = Array(parfaitDir.appendingPathComponent("diagnostics.json").path.utf8CString)
    ...
    private static func writeRecord(kind: String, detail: String, callstack: [String]) {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: payload, options: [.prettyPrinted, .sortedKeys]) {
            let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            let parfaitDir = base.appendingPathComponent("Parfait", isDirectory: true)
            try? data.write(to: parfaitDir.appendingPathComponent("diagnostics.json"), options: .atomic)
    ...
    static func allCrashes() -> [CrashRecord] {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        let crashesDir = base.appendingPathComponent("Parfait/Crashes", isDirectory: true)
