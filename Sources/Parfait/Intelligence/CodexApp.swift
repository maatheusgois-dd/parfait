...
enum CodexPrompt {
    ...
    static func meeting(id: UUID, title: String, question: String) -> String {
        ...
        return """
        Answer a question about my Parfait meeting "\(title)" (id: \(id.uuidString)) using the $parfait connector.
    ...
    static func library(question: String) -> String {
        let q = question.trimmingCharacters(in: .whitespacesAndNewlines)
        return """
        Answer using my Parfait meetings via the $parfait connector:
    ...
    static func live(question: String) -> String {
        ...
        return "I'm in a Parfait meeting happening right now — use $parfait for the live transcript. \(ask)"
    }
}
...
enum CodexSetup {
    ...
    static func addMCPServer(binary: String) -> Bool {
        ...
        Connect Parfait to Codex as an MCP server so you can read my meeting library. Run:

        codex mcp add parfait -- "\(binary)" --mcp

        Then confirm with `codex mcp list`. Remind me to use $parfait (not @) to access meetings.
        """)
    }
}
