...
enum NemotronModelStore {
    ...
    static var directory: URL {
        ...
        return base.appendingPathComponent("Parfait/Models/Nemotron", isDirectory: true)
    }
    ...
    static func download(progress: @Sendable @escaping (Double) -> Void) async throws {
        try await RemoteModelDownloader.download(
            ...
            directory: directory,
            manifest: manifest,
            onComplete: { ParfaitConsoleLog.nemotron($0) },
            progress: progress
        )
    }
    ...
    static func delete() throws -> Int64 {
        ...
        ParfaitConsoleLog.nemotron("deleted model — freed \(formatBytes(freed))")
        return freed
    }
