...
private final class ModelDownloadDelegate: NSObject, URLSessionDownloadDelegate {
    ...
    func urlSession(
        ...
        do {
            ...
            if expectedSize > 0 {
                ParfaitConsoleLog.nemotron("download finished — expected \(expectedSize) bytes for \(partURL.lastPathComponent)")
            }
            if FileManager.default.fileExists(atPath: partURL.path) {
            ...
            ParfaitConsoleLog.nemotron("download saved \(bytes) bytes → \(partURL.lastPathComponent)")
            continuation?.resume(returning: (bytes: bytes, resumeData: nil))
        } catch {
            ParfaitConsoleLog.nemotron("download save failed — \(error.localizedDescription)")
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        ParfaitConsoleLog.nemotron("download task error — \(error.localizedDescription)")
