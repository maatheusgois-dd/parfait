...
/// into `~/Library/Application Support/Parfait/Models/Soniqo/`. Inference is not
...
enum SoniqoModelStore {
    static func directory(for model: SoniqoModel) -> URL {
        ...
        return base.appendingPathComponent("Parfait/Models/Soniqo/\(model.folderName)", isDirectory: true)
    }
    ...
    static func download(
        ...
        try await RemoteModelDownloader.download(
            ...
            onComplete: { ParfaitConsoleLog.soniqo($0) },
            progress: progress
        )
    }
    ...
    static func delete(_ model: SoniqoModel) throws -> Int64 {
        ...
        ParfaitConsoleLog.soniqo("deleted model — freed \(formatBytes(freed))")
        return freed
    }
