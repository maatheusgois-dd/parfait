...
enum TranscriptionLocales {
    ...
    static func primary() async -> Locale? {
        for raw in primaryCandidateIdentifiers() {
            if let resolved = await SpeechTranscriber.supportedLocale(
                equivalentTo: Locale(identifier: raw)) {
                ParfaitConsoleLog.locales("primary locale → \(resolved.identifier(.bcp47))")
                return resolved
            }
        }
        let fallback = (await supportedCandidates()).first
        if let fallback {
            ParfaitConsoleLog.locales("primary fallback → \(fallback.identifier(.bcp47))")
        } else {
            ParfaitConsoleLog.locales("no supported locale found")
        }
        return fallback
    }
    ...
    static func ensureModels(progress: (@Sendable (Double) -> Void)? = nil) async throws {
        ...
        if !missing.isEmpty {
            ParfaitConsoleLog.locales("downloading models: \(missing.map { $0.identifier(.bcp47) }.joined(separator: ", "))")
        } else {
            ParfaitConsoleLog.locales("all speech models installed")
        }
