...
/// `~/Library/Application Support/Parfait/Crashes/`), written by the opt-in crash
...
struct CrashHistoryPanel: View {
    ...
    private func toolbarButton(
        ...
        Button(action: action) {
            HStack(spacing: 3) {
                Image(systemName: icon).font(.parfait(9))
                Text(label).font(.parfait(10))
            }
    ...
    private func detailHeader(for record: CrashDiagnosticLog.CrashRecord) -> some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            Button {
                ...
                HStack(spacing: 3) {
                    Image(systemName: "doc.on.doc").font(.parfait(9))
                    Text("Copy").font(.parfait(10))
                }
                .foregroundStyle(Theme.blueberry(scheme))
            }
            .buttonStyle(.plain)
